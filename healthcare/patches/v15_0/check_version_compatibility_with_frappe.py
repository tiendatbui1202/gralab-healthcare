...
def execute():
	...
	if frappe_v.startswith("14") and healthcare_v.startswith("15"):
		message = f"""
			The `develop` branch of Marley Health is no longer compatible with Gralab & ERPNext's `version-14`.
			Since you are using ERPNext/Gralab `version-14` please switch Marley Health app's branch to `version-14` and then proceed with the update.\n\t
